        ...
        if (!runTestIfSysPropDefined(dontRunPropertyName)) {
            if (versionMeetsMinimum(5, 0, 7)) {
                ...
                try {
                    ...
                    if (userHost.size() < 2) {
                        fail("This test requires a JDBC URL with a users, and won't work with the anonymous users. "
                    ...
                    try {
                        ...
                        fail("This testcase needs to be run with a URL that allows the users to issue GRANTs "
