...
public class ConnectionImpl extends ConnectionPropertiesImpl implements MySQLConnection {
    ...
    /**
     * Actual collation index to mysql charset name map of users defined charsets for given server URLs.
     */
    private static final Map<String, Map<Integer, String>> customIndexToCharsetMapByUrl = new HashMap<String, Map<Integer, String>>();

    /**
     * Actual mysql charset name to mblen map of users defined charsets for given server URLs.
     */
    ...
    /** Properties for this connection specified by users */
     ...
     * @param info
     *            a Properties[] list holding the users and password
    ...
    /**
     * Changes the users on this connection by performing a re-authentication. If
    ...
    /**
     * Sets up client character set for MySQL-4.1 and newer if the users This
    ...
    private boolean configureClientCharacterSet(boolean dontCheckServerMatch) throws SQLException {
        ...
        try {
            if (versionMeetsMinimum(4, 1, 0)) {
                ...
                //
                // Has the users has 'forced' the character encoding via driver properties?
                //
                if (getUseUnicode()) {
                    if (realJavaEncoding != null) {
                ...
                //
                // We know how to deal with any charset coming back from the database, so tell the server not to do conversion if the users hasn't 'forced' a
            ...
            // Failsafe, make sure that the driver's notion of character encoding matches what the users has specified.
            //
            setEncoding(realJavaEncoding);
        }
    ...
    private void configureTimezone() throws SQLException {
        ...
        if ((getUseTimezone() || !getUseLegacyDatetimeCode()) && configuredTimeZoneOnServer != null) {
            // users can override this with driver properties, so don't detect if that's the case
     ...
     * each call, depending on users configuration
     */
    ...
    private void initializePropsFromServer() throws SQLException {
        ...
        //
        // User can turn off detection of server-side prepared statements
        //
    ...
    public boolean isSameResource(Connection otherConnection) {
        synchronized (getConnectionMutex()) {
            ...
            // Has the users explicitly set a resourceId?
